...
function Accounts({ accounts, connections }: { accounts: Row[]; connections: Row[] }) {
  ...
  return <section className="panel accounts"><header><div><p className="eyebrow">CONNECTIONS</p><h2>账户状态</h2></div></header>{rows.length ? rows.map((account, index) => { const matched = connections.find(connection => connection.account_id === account.account_id); const state = String(matched?.state ?? account.state ?? account.data_state ?? "CONNECTED"); const failure = String(matched?.error ?? account.error ?? ""); const tone = failure || ["ERROR", "DISCONNECTED"].includes(state) ? "error" : state === "STALE" || state === "WARNING" ? "warning" : "live"; return <div className="account" key={String(account.account_id ?? index)}><div><b>{String(account.account_name ?? account.name ?? account.exchange)}</b><p>{failure ? `同步失败：${failure}` : account.account_equity ? `${fmt(String(account.account_equity), true)} USDT` : "等待快照"}</p></div><span className={`account-status ${tone}`}>{failure ? "异常" : cnState(state)}</span></div>; }) : <Empty text="没有已连接账户。" />}</section>;
}
