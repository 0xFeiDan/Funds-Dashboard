...
from .http import dec,request,unix_ms
...
class BitgetAdapter(ExchangeAdapter):
    ...
    async def get_account_summary(self)->NormalizedAccountSummary:
        rows=await self._get("/api/v2/mix/account/accounts",{"productType":self.product_type}); row=next((x for x in rows if dec(x.get("accountEquity"))!=0),rows[0] if rows else {})
        now=datetime.now(timezone.utc); equity=dec(row.get("accountEquity")); maint=dec(row.get("crossedRiskRate"))
        return NormalizedAccountSummary(exchange=Exchange.BITGET,account_id=self.account_id,account_name=self.account_name,margin_currency=row.get("marginCoin","USD"),wallet_balance=dec(row.get("usdtEquity") or row.get("accountEquity")),account_equity=equity,available_balance=dec(row.get("available")),unrealized_pnl=dec(row.get("unrealizedPL")),initial_margin=dec(row.get("locked")),maintenance_margin=ZERO,margin_ratio=maint if maint else None,total_position_notional=ZERO,updated_at=now,data_source=DataSource.REST,raw_values={k:str(row.get(k,"")) for k in ("accountEquity","available","unrealizedPL")},field_notes={"account_equity":"Bitget accountEquity. Product type is isolated by adapter configuration."})
    async def get_positions(self)->list[NormalizedPosition]:
        rows=await self._get("/api/v2/mix/position/all-position",{"productType":self.product_type}); now=datetime.now(timezone.utc); result=[]
        for row in rows:
